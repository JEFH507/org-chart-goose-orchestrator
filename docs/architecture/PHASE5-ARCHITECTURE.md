# Phase 5 Architecture — Org-Chart goose Orchestrator

**Date:** 2025-11-06  
**Version:** v0.5.0  
**Status:** Production-Ready (Dev Mode)

This document provides comprehensive architectural views of the Phase 5 system, showing all users, UIs, middleware, controller, and data flows.

---

## 📋 Table of Contents

1. [System Overview](#system-overview)
2. [View 1: Complete System Architecture](#view-1-complete-system-architecture)
3. [View 2: User Workflows by Role](#view-2-user-workflows-by-role)
4. [View 3: Data Flow - Profile Loading](#view-3-data-flow---profile-loading)
5. [View 4: Data Flow - Profile Publishing with Vault](#view-4-data-flow---profile-publishing-with-vault)
6. [Component Details](#component-details)
7. [API Endpoint Summary](#api-endpoint-summary)
8. [Security Architecture](#security-architecture)
9. [Phase 6 Preview](#phase-6-preview)

---

## System Overview

**Phase 5 Capabilities:**
- ✅ **Role-Based Profiles**: 6 pre-configured roles (finance, legal, developer, hr, executive, support)
- ✅ **Organizational Chart Management**: CSV import, hierarchical tree API
- ✅ **Vault Integration**: HMAC profile signing (dev mode, production path documented)
- ✅ **Profile Configuration Generation**: Dynamic config.yaml, goosehints, gooseignore
- ✅ **JWT Authentication**: Keycloak OIDC integration with role extraction
- ✅ **Privacy Guard**: Local PII masking (Phase 2, integrated)

**Deployment Mode:**
- Development (current): HTTP, root token, in-memory storage
- Production (Phase 6): HTTPS, AppRole, persistent storage, audit logging

---

## View 1: Complete System Architecture

### Full System Topology

```
┌────────────────────────────────────────────────────────────────────────────┐
│                           ENTERPRISE USERS                                  │
│                                                                             │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  │
│  │   Finance    │  │    Legal     │  │  Developer   │  │      HR      │  │
│  │    User      │  │    User      │  │    User      │  │    User      │  │
│  │              │  │              │  │              │  │              │  │
│  │ Profile:     │  │ Profile:     │  │ Profile:     │  │ Profile:     │  │
│  │ finance      │  │ legal        │  │ developer    │  │ hr           │  │
│  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  │
│         │                  │                  │                  │          │
│         │                  │                  │                  │          │
└─────────┼──────────────────┼──────────────────┼──────────────────┼──────────┘
          │                  │                  │                  │
          │                  ↓                  │                  │
          │         ┌────────────────┐          │                  │
          │         │   ADMIN USER   │          │                  │
          │         │   (IT/Admin)   │          │                  │
          │         │                │          │                  │
          │         │  Manages:      │          │                  │
          │         │  - Profiles    │          │                  │
          │         │  - Org Chart   │          │                  │
          │         │  - Publishing  │          │                  │
          │         └───────┬────────┘          │                  │
          │                 │                   │                  │
          ↓                 ↓                   ↓                  ↓
┌────────────────────────────────────────────────────────────────────────────┐
│                              USER INTERFACES                                │
│                                                                             │
│  ┌──────────────────────────────────┐  ┌──────────────────────────────┐   │
│  │      GOOSE DESKTOP APP           │  │    ADMIN WEB UI (Phase 6)    │   │
│  │      (Open Source v1.12)         │  │    (SvelteKit - Planned)     │   │
│  │                                  │  │                              │   │
│  │  Features:                       │  │  Features:                   │   │
│  │  • Chat interface                │  │  • Dashboard (D3.js)         │   │
│  │  • MCP extensions                │  │  • Profile editor (Monaco)   │   │
│  │  • Provider management           │  │  • Org chart manager (CSV)   │   │
│  │  • Session history               │  │  • Audit logs (table)        │   │
│  │  • Profile-based config          │  │  • User sessions (live)      │   │
│  │                                  │  │  • Vault status              │   │
│  │  Auto-configured from:           │  │                              │   │
│  │  ~/.config/goose/config.yaml     │  │  Authenticated via:          │   │
│  │  ~/.config/goose/.goosehints     │  │  Keycloak OIDC (admin role)  │   │
│  │  ~/.config/goose/.gooseignore    │  │                              │   │
│  └───────────────┬──────────────────┘  └────────────┬─────────────────┘   │
│                  │                                   │                     │
└──────────────────┼───────────────────────────────────┼─────────────────────┘
                   │                                   │
                   │                                   │
                   ↓                                   ↓
┌────────────────────────────────────────────────────────────────────────────┐
│                         AUTHENTICATION LAYER                                │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐ │
│  │                    KEYCLOAK (OIDC Provider)                          │ │
│  │                    Port: 8080 | Realm: dev                           │ │
│  │                                                                      │ │
│  │  Flows:                                                              │ │
│  │  1. User Login → Password Grant → JWT Token                         │ │
│  │  2. Token contains claims: {sub, email, roles: ["finance"]}         │ │
│  │  3. Controller extracts role from JWT for profile lookup            │ │
│  │                                                                      │ │
│  │  Users Configured:                                                   │ │
│  │  • testuser / testpassword (finance role)                           │ │
│  │  • admin@example.com (admin role)                                   │ │
│  │                                                                      │ │
│  │  Clients:                                                            │ │
│  │  • goose-controller (backend client)                                │ │
│  │  • admin-ui (frontend client, Phase 6)                              │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
└─────────────────────────────────┬───────────────────────────────────────────┘
                                  │
                                  ↓ JWT Token (Bearer)
┌────────────────────────────────────────────────────────────────────────────┐
│                         MIDDLEWARE LAYER                                    │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐ │
│  │               PRIVACY GUARD MCP (Phase 2 - Integrated)               │ │
│  │               Port: 8089 | Language: Rust                            │ │
│  │                                                                      │ │
│  │  Capabilities:                                                       │ │
│  │  • PII Detection: 8 entity types (SSN, Email, Phone, CC, etc.)      │ │
│  │  • Masking: Deterministic HMAC-SHA256 pseudonymization              │ │
│  │  • Format-Preserving: Phone/SSN maintain format                     │ │
│  │  • Performance: P50=16ms, P95=22ms                                  │ │
│  │                                                                      │ │
│  │  Endpoints:                                                          │ │
│  │  • POST /guard/scan       → Detect PII entities                     │ │
│  │  • POST /guard/mask       → Mask PII (forward to LLM)               │ │
│  │  • POST /guard/reidentify → Restore original PII                    │ │
│  │  • GET  /guard/status     → Health check                            │ │
│  │  • POST /guard/flush      → Clear session tokens                    │ │
│  │                                                                      │ │
│  │  Privacy Flow:                                                       │ │
│  │  User Text → Scan → Mask → LLM (masked) → Response → Unmask → User  │ │
│  │                                                                      │ │
│  │  Storage:                                                            │ │
│  │  • Local tokens: ~/.goose/pii-tokens/ (ephemeral)                   │ │
│  │  • No cloud storage (privacy-first)                                 │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
└─────────────────────────────────┬───────────────────────────────────────────┘
                                  │
                                  ↓
┌────────────────────────────────────────────────────────────────────────────┐
│                        ORCHESTRATION LAYER                                  │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐ │
│  │                 CONTROLLER API (Rust/Axum)                           │ │
│  │                 Port: 8088 | Language: Rust                          │ │
│  │                                                                      │ │
│  │  ┌────────────────────────────────────────────────────────────────┐ │ │
│  │  │  PROFILE ENDPOINTS (Workstream D)                              │ │ │
│  │  │                                                                │ │ │
│  │  │  User Endpoints (Authenticated):                               │ │ │
│  │  │  • GET /profiles/{role}                → Full profile JSON     │ │ │
│  │  │  • GET /profiles/{role}/config         → config.yaml          │ │ │
│  │  │  • GET /profiles/{role}/goosehints     → .goosehints          │ │ │
│  │  │  • GET /profiles/{role}/goosehints/local → Local hints        │ │ │
│  │  │  • GET /profiles/{role}/gooseignore    → .gooseignore         │ │ │
│  │  │  • GET /profiles/{role}/recipes        → Recipe list          │ │ │
│  │  │                                                                │ │ │
│  │  │  Admin Endpoints (Admin role required):                        │ │ │
│  │  │  • POST /admin/profiles                → Create profile        │ │ │
│  │  │  • PUT  /admin/profiles/{role}         → Update profile        │ │ │
│  │  │  • POST /admin/profiles/{role}/publish → Sign with Vault       │ │ │
│  │  └────────────────────────────────────────────────────────────────┘ │ │
│  │                                                                      │ │
│  │  ┌────────────────────────────────────────────────────────────────┐ │ │
│  │  │  ORG CHART ENDPOINTS (Workstream E)                            │ │ │
│  │  │                                                                │ │ │
│  │  │  Admin Endpoints:                                              │ │ │
│  │  │  • POST /admin/org/import  → Upload CSV (bulk insert)         │ │ │
│  │  │  • GET  /admin/org/imports → Import history + audit           │ │ │
│  │  │  • GET  /admin/org/tree    → Hierarchical org structure       │ │ │
│  │  │                                                                │ │ │
│  │  │  CSV Format:                                                   │ │ │
│  │  │  employee_id,email,name,manager_email,department,title        │ │ │
│  │  │                                                                │ │ │
│  │  │  Tree Output: Nested JSON (manager → [reports])               │ │ │
│  │  └────────────────────────────────────────────────────────────────┘ │ │
│  │                                                                      │ │
│  │  ┌────────────────────────────────────────────────────────────────┐ │ │
│  │  │  INFRASTRUCTURE ENDPOINTS                                      │ │ │
│  │  │                                                                │ │ │
│  │  │  • GET  /status           → Health check                       │ │ │
│  │  │  • POST /audit/ingest     → Audit log ingestion (Phase 1)     │ │ │
│  │  └────────────────────────────────────────────────────────────────┘ │ │
│  │                                                                      │ │
│  │  Middleware:                                                         │ │
│  │  • JWT Verification (RS256 + JWKS from Keycloak)                   │ │
│  │  • Role Extraction (from JWT claims)                                │ │
│  │  • Request Logging (structured, PII-redacted)                       │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
└─────────────┬───────────────────────────────────┬───────────────────────────┘
              │                                   │
              ↓                                   ↓
┌──────────────────────────────┐   ┌──────────────────────────────────────────┐
│    VAULT (HMAC Signing)      │   │       POSTGRESQL DATABASE                │
│    Port: 8200 (HTTP)         │   │       Port: 5432                         │
│    Mode: Dev (Phase 5)       │   │                                          │
│                              │   │  ┌────────────────────────────────────┐  │
│  Transit Engine:             │   │  │  profiles (JSONB storage)          │  │
│  • Key: profile-signing      │   │  │  ┌──────────────────────────────┐  │  │
│  • Algorithm: HMAC-SHA256    │   │  │  │ role VARCHAR(255) PK         │  │  │
│  • Operations: sign, verify  │   │  │  │ display_name VARCHAR(255)    │  │  │
│  │                            │   │  │  │ description TEXT             │  │  │
│  Endpoints Used:             │   │  │  │ config JSONB                 │  │  │
│  • POST /transit/hmac/...    │   │  │  │   ├─ providers               │  │  │
│  │                            │   │  │  │   ├─ extensions              │  │  │
│  Signature Format:           │   │  │  │   ├─ goosehints              │  │  │
│  vault:v1:BASE64_HMAC        │   │  │  │   ├─ gooseignore             │  │  │
│  │                            │   │  │  │   ├─ recipes                 │  │  │
│  Environment:                │   │  │  │   ├─ automated_tasks          │  │  │
│  • VAULT_ADDR (HTTP)         │   │  │  │   ├─ policies                │  │  │
│  • VAULT_TOKEN (root)        │   │  │  │   ├─ privacy                 │  │  │
│  │                            │   │  │  │   ├─ env_vars                │  │  │
│  Phase 6 Production:         │   │  │  │   └─ signature (Vault HMAC)  │  │  │
│  • TLS/HTTPS (2h)            │   │  │  │ created_at TIMESTAMPTZ       │  │  │
│  • AppRole auth (3h)         │   │  │  │ updated_at TIMESTAMPTZ       │  │  │
│  • Persistent storage (2h)   │   │  │  └──────────────────────────────┘  │  │
│  • Audit device (1h)         │   │  │  GIN index on config (JSONB ops)   │  │
│  • Signature verify (2h)     │   │  └────────────────────────────────────┘  │
└──────────────────────────────┘   │                                          │
                                   │  ┌────────────────────────────────────┐  │
                                   │  │  org_users (Org chart)             │  │
                                   │  │  ┌──────────────────────────────┐  │  │
                                   │  │  │ employee_id VARCHAR(50) PK   │  │  │
                                   │  │  │ email VARCHAR(255) UNIQUE    │  │  │
                                   │  │  │ name VARCHAR(255)            │  │  │
                                   │  │  │ manager_email VARCHAR(255)   │  │  │
                                   │  │  │   (self-referencing FK)      │  │  │
                                   │  │  │ department VARCHAR(255)      │  │  │
                                   │  │  │ title VARCHAR(255)           │  │  │
                                   │  │  │ created_at TIMESTAMPTZ       │  │  │
                                   │  │  └──────────────────────────────┘  │  │
                                   │  │  Indexes:                          │  │
                                   │  │  • email                           │  │
                                   │  │  • manager_email                   │  │
                                   │  │  • department                      │  │
                                   │  └────────────────────────────────────┘  │
                                   │                                          │
                                   │  ┌────────────────────────────────────┐  │
                                   │  │  org_imports (Audit trail)         │  │
                                   │  │  ┌──────────────────────────────┐  │  │
                                   │  │  │ import_id SERIAL PK          │  │  │
                                   │  │  │ filename VARCHAR(255)        │  │  │
                                   │  │  │ rows_imported INT            │  │  │
                                   │  │  │ imported_by VARCHAR(255)     │  │  │
                                   │  │  │   (from JWT email)           │  │  │
                                   │  │  │ imported_at TIMESTAMPTZ      │  │  │
                                   │  │  └──────────────────────────────┘  │  │
                                   │  └────────────────────────────────────┘  │
                                   │                                          │
                                   │  ┌────────────────────────────────────┐  │
                                   │  │  Legacy Tables (Phase 1-2)         │  │
                                   │  │  • audit_events (Phase 1)          │  │
                                   │  │  • sessions (Phase 1)              │  │
                                   │  │  • privacy_events (Phase 2)        │  │
                                   │  └────────────────────────────────────┘  │
                                   └──────────────────────────────────────────┘

┌────────────────────────────────────────────────────────────────────────────┐
│                         EXTERNAL SERVICES                                   │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐ │
│  │                      CLOUD LLM PROVIDERS                             │ │
│  │                                                                      │ │
│  │  • OpenAI (GPT-4, GPT-3.5)                                          │ │
│  │  • Anthropic (Claude 3.5 Sonnet, Claude 3 Opus)                     │ │
│  │  • Google (Gemini Pro)                                              │ │
│  │  • OpenRouter (multi-provider gateway)                              │ │
│  │                                                                      │ │
│  │  Privacy Protection:                                                 │ │
│  │  All requests pass through Privacy Guard MCP                         │ │
│  │  LLMs receive masked PII only (EMAIL_7a3f9b, not john@acme.com)     │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
│                                                                             │
│  ┌──────────────────────────────────────────────────────────────────────┐ │
│  │                      LOCAL LLM (OLLAMA)                              │ │
│  │                      Port: 11434                                     │ │
│  │                                                                      │ │
│  │  Models Available:                                                   │ │
│  │  • qwen2.5:3b (Privacy Guard NER, Phase 2.2)                        │ │
│  │  • llama3.2:3b (local inference)                                    │ │
│  │  • mistral:7b (local inference)                                     │ │
│  │                                                                      │ │
│  │  Use Case:                                                           │ │
│  │  Legal/Finance profiles can enforce local-only (no cloud PII)       │ │
│  └──────────────────────────────────────────────────────────────────────┘ │
└────────────────────────────────────────────────────────────────────────────┘
```

**Key Architecture Principles:**
1. **Layered Security**: Keycloak → JWT → Controller → Vault → Database
2. **Privacy-First**: All PII passes through Privacy Guard before cloud
3. **Profile-Driven**: User role determines complete goose configuration
4. **Signature Integrity**: Vault HMAC prevents profile tampering
5. **Org-Aware**: Hierarchical org chart enables future approval workflows

---

## View 2: User Workflows by Role

### 2.1 Finance User Workflow

```
┌──────────────────────────────────────────────────────────────────────┐
│                    FINANCE USER: Budget Analysis                      │
└──────────────────────────────────────────────────────────────────────┘

Step 1: Authentication
┌─────────────┐
│ Finance     │  Login via Keycloak OIDC
│ User        │  ──────────────────────────────────┐
│ (Desktop)   │                                    │
└─────────────┘                                    ↓
                                         ┌──────────────────┐
                                         │   Keycloak       │
                                         │   Returns JWT:   │
                                         │   {              │
                                         │     role: "fin"  │
                                         │     email: "..." │
                                         │   }              │
                                         └──────────────────┘

Step 2: Profile Loading
┌─────────────┐
│ goose       │  GET /profiles/finance (with JWT)
│ Desktop     │  ──────────────────────────────────┐
│             │                                    │
└─────────────┘                                    ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  1. Verify JWT   │
                                         │  2. Extract role │
                                         │  3. Query DB     │
                                         │  4. Return JSON  │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  Profile JSON:   │
                                         │  {               │
                                         │    providers: {  │
                                         │      primary:    │
                                         │       "claude-3.5│
                                         │        -sonnet"  │
                                         │    },            │
                                         │    extensions: [ │
                                         │      {name:      │
                                         │       "github"}  │
                                         │    ],            │
                                         │    privacy: {    │
                                         │      mode:       │
                                         │       "strict"   │
                                         │    }             │
                                         │  }               │
                                         └──────────────────┘

Step 3: Configuration Generation
┌─────────────┐
│ goose       │  GET /profiles/finance/config
│ Desktop     │  GET /profiles/finance/goosehints
│             │  GET /profiles/finance/gooseignore
│             │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  Generates:      │
                                         │  • config.yaml   │
                                         │  • .goosehints   │
                                         │  • .gooseignore  │
                                         │  from profile    │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
┌─────────────┐                        ┌──────────────────┐
│ Local       │  Saves to:             │  Files:          │
│ Filesystem  │  ~/.config/goose/      │  • config.yaml   │
│             │  <────────────────────── • .goosehints    │
│             │                         │  • .gooseignore  │
└─────────────┘                        └──────────────────┘

Step 4: Task Execution (with Privacy Guard)
┌─────────────┐
│ User types: │  "Analyze Q4 budget for john.doe@acme.com"
│ goose Chat  │  ────────────────────────────────────┐
└─────────────┘                                      │
                                                     ↓
                                         ┌──────────────────┐
                                         │ Privacy Guard    │
                                         │ 1. Scan PII:     │
                                         │    EMAIL detected│
                                         │ 2. Mask:         │
                                         │    → EMAIL_7a3f9b│
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ Claude API       │
                                         │ Receives:        │
                                         │ "Analyze budget  │
                                         │  for EMAIL_7a3f9b│
                                         │  ..."            │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ Response:        │
                                         │ "EMAIL_7a3f9b has│
                                         │  overspent by 20%│
┌─────────────┐                        └────────┬─────────┘
│ Privacy     │  Unmask EMAIL_7a3f9b            │
│ Guard       │  <──────────────────────────────┘
│             │
│ Returns:    │
│ "john.doe@  │  ─────────────────────────────────┐
│  acme.com   │                                   │
│  has over-  │                                   ↓
│  spent by   │                         ┌──────────────────┐
│  20%"       │                         │  User sees       │
└─────────────┘                         │  original text   │
                                        │  (unmasked)      │
                                        └──────────────────┘

Profile Features Applied:
✅ Primary provider: Claude 3.5 Sonnet (from finance profile)
✅ Extensions: github (budget tracking in GitHub issues)
✅ Privacy: Strict PII protection (EMAIL masked before cloud)
✅ Goosehints: Finance-specific prompts ("Always show currency")
✅ Gooseignore: Protects .env files, secrets
```

---

### 2.2 Legal User Workflow (Local-Only Enforcement)

```
┌──────────────────────────────────────────────────────────────────────┐
│                LEGAL USER: Contract Review (Attorney-Client Privilege)│
└──────────────────────────────────────────────────────────────────────┘

Step 1: Profile Loading
┌─────────────┐
│ Legal User  │  GET /profiles/legal (JWT)
│ Desktop     │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Profile JSON:   │
                                         │  {               │
                                         │    providers: {  │
                                         │      primary:    │
                                         │       "ollama/   │
                                         │        llama3.2" │
                                         │      forbidden:  │
                                         │       ["openai", │
                                         │        "anthro"] │
                                         │    },            │
                                         │    privacy: {    │
                                         │      local_only: │
                                         │       true,      │
                                         │      retention:  │
                                         │       0          │
                                         │    },            │
                                         │    policies: [   │
                                         │      {deny_tool: │
                                         │       "developer_│
                                         │        _shell"}  │
                                         │    ]             │
                                         │  }               │
                                         └──────────────────┘

Step 2: Task Execution (Local Only)
┌─────────────┐
│ User types: │  "Review contract for Acme Corp, CEO John Doe"
│ goose Chat  │  ────────────────────────────────────┐
└─────────────┘                                      │
                                                     ↓
                                         ┌──────────────────┐
                                         │ Privacy Guard    │
                                         │ Policy:          │
                                         │ local_only=true  │
                                         │                  │
                                         │ 1. Scan PII:     │
                                         │    PERSON, ORG   │
                                         │ 2. Mask locally  │
                                         │ 3. Block cloud   │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ Ollama (Local)   │
                                         │ Model: llama3.2  │
                                         │ Receives:        │
                                         │ "Review contract │
                                         │  for ORG_A,      │
                                         │  CEO PERSON_B"   │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ Response:        │
                                         │ "ORG_A contract  │
                                         │  complies with   │
                                         │  terms. PERSON_B │
                                         │  signature req." │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
┌─────────────┐                        ┌──────────────────┐
│ Privacy     │  Unmask locally        │  User sees:      │
│ Guard       │  ────────────────────→ │  "Acme Corp      │
│             │                         │   contract ok.   │
│             │                         │   John Doe sign  │
│             │                         │   required."     │
└─────────────┘                        └──────────────────┘

Privacy Guarantees:
✅ No cloud provider used (Ollama local only)
✅ PII masked even for local model (defense in depth)
✅ Memory retention: 0 days (ephemeral)
✅ Code execution blocked (deny_tool: "developer__shell")
✅ Attorney-client privilege protected

Phase 6 Enhancement:
⏳ Real-time enforcement: Block cloud API calls at network level
⏳ Admin audit: Log all local-only profile usage
```

---

### 2.3 Admin Workflow: Profile Publishing

```
┌──────────────────────────────────────────────────────────────────────┐
│               ADMIN WORKFLOW: Create and Publish Profile              │
└──────────────────────────────────────────────────────────────────────┘

Step 1: Create Profile (D7)
┌─────────────┐
│ Admin       │  POST /admin/profiles
│ (Web UI or  │  Authorization: Bearer <JWT with admin role>
│  curl)      │  Content-Type: application/json
│             │  
│             │  {
│             │    "role": "finance",
│             │    "display_name": "Finance Manager",
│             │    "description": "Budget analysis",
│             │    "providers": {
│             │      "primary": "claude-3.5-sonnet"
│             │    },
│             │    "privacy": {
│             │      "mode": "strict"
│             │    }
│             │  }
│             │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  1. Verify JWT   │
                                         │  2. Check role=  │
                                         │     admin        │
                                         │  3. Validate     │
                                         │     schema       │
                                         │  4. Insert DB    │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  PostgreSQL      │
                                         │  INSERT INTO     │
                                         │  profiles        │
                                         │  (role, config)  │
                                         │  VALUES          │
                                         │  ('finance',     │
                                         │   {...})         │
                                         │                  │
                                         │  signature: NULL │
                                         │  (unsigned)      │
                                         └──────────────────┘

Step 2: Update Profile (D8)
┌─────────────┐
│ Admin       │  PUT /admin/profiles/finance
│             │  Authorization: Bearer <JWT>
│             │  
│             │  {
│             │    "privacy": {
│             │      "retention_days": 30
│             │    }
│             │  }
│             │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  1. Fetch profile│
                                         │  2. Merge JSON   │
                                         │     (partial     │
                                         │      update)     │
                                         │  3. Update DB    │
                                         │  4. Clear sig    │
                                         │     (invalidate) │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  PostgreSQL      │
                                         │  UPDATE profiles │
                                         │  SET config =    │
                                         │    merge(old,new)│
                                         │      signature = │
                                         │       NULL       │
                                         │  WHERE role =    │
                                         │   'finance'      │
                                         └──────────────────┘

Step 3: Publish Profile (D9 - Vault Signing)
┌─────────────┐
│ Admin       │  POST /admin/profiles/finance/publish
│             │  Authorization: Bearer <JWT>
│             │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  Publish Flow:   │
                                         │                  │
                                         │  1. Fetch profile│
                                         │     from DB      │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  2. Serialize    │
                                         │     to JSON      │
                                         │     (canonical)  │
                                         │                  │
                                         │  sorted_json =   │
                                         │  serde_json::to_ │
                                         │  string_pretty   │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  3. Send to Vault│
                                         │                  │
                                         │  POST /v1/transit│
                                         │  /hmac/profile-  │
                                         │  signing/sha2-256│
                                         │                  │
                                         │  {input: BASE64} │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  Vault Transit   │
                                         │  Engine          │
                                         │                  │
                                         │  1. Decode BASE64│
                                         │  2. Compute HMAC-│
                                         │     SHA256 with  │
                                         │     key          │
                                         │  3. Return sig   │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  Response:       │
                                         │  {               │
                                         │    signature:    │
                                         │    "vault:v1:    │
                                         │     6wmfS0Vo91Ga0│
                                         │     E9BkInhWZvLJ3│
                                         │     qQodEnXhykdy│
                                         │     wB8kc="      │
                                         │  }               │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  Controller      │
                                         │  4. Build sig    │
                                         │     metadata     │
                                         │                  │
                                         │  signature = {   │
                                         │    algorithm:    │
                                         │     "sha2-256",  │
                                         │    vault_key:    │
                                         │     "transit/...",
                                         │    signed_at:    │
                                         │     "2025-11-06",│
                                         │    signed_by:    │
                                         │     "admin@...", │
                                         │    signature:    │
                                         │     "vault:v1:..."
                                         │  }               │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  5. Update DB    │
                                         │                  │
                                         │  UPDATE profiles │
                                         │  SET config =    │
                                         │   jsonb_set(     │
                                         │     config,      │
                                         │     '{signature}'│
                                         │     signature    │
                                         │   )              │
                                         │  WHERE role =    │
                                         │   'finance'      │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │  Response:       │
                                         │  {               │
                                         │    message:      │
                                         │     "Published", │
                                         │    signature: {  │
                                         │      ...         │
                                         │    }             │
                                         │  }               │
                                         └──────────────────┘

Signature Benefits:
✅ Tamper detection: Any manual DB edit invalidates signature
✅ Integrity: Vault key protection (Phase 6: AppRole + Raft storage)
✅ Audit: Signed by whom, when (metadata stored in profile)
✅ Verification: Phase 6 adds verify on load (reject if invalid)

Phase 6 Production Upgrade:
⏳ TLS/HTTPS (VAULT_ADDR = https://vault:8200)
⏳ AppRole authentication (remove VAULT_TOKEN)
⏳ Persistent Raft storage (unseal procedure)
⏳ File audit device (/vault/logs)
⏳ Signature verification on profile load (403 if tampered)
```

---

### 2.4 Developer User Workflow

```
┌──────────────────────────────────────────────────────────────────────┐
│              DEVELOPER USER: Code Review with Tools                   │
└──────────────────────────────────────────────────────────────────────┘

Step 1: Profile Loading
┌─────────────┐
│ Developer   │  GET /profiles/developer (JWT)
│ User        │  ──────────────────────────────────┐
└─────────────┘                                    │
                                                   ↓
                                         ┌──────────────────┐
                                         │  Profile JSON:   │
                                         │  {               │
                                         │    providers: {  │
                                         │      primary:    │
                                         │       "gpt-4o",  │
                                         │      worker:     │
                                         │       "gpt-4o-   │
                                         │        mini"     │
                                         │    },            │
                                         │    extensions: [ │
                                         │      {name:      │
                                         │       "github"}, │
                                         │      {name:      │
                                         │       "developer"│
                                         │        tools: [  │
                                         │         "shell", │
                                         │         "analyze"│
                                         │        ]}        │
                                         │    ],            │
                                         │    policies: [   │
                                         │      {allow_tool:│
                                         │       "developer_│
                                         │        _shell"}  │
                                         │    ],            │
                                         │    goosehints: { │
                                         │      global:     │
                                         │       "Use TDD"  │
                                         │    }             │
                                         │  }               │
                                         └──────────────────┘

Step 2: Task Execution (with MCP Tools)
┌─────────────┐
│ User types: │  "Review PR #123 from repo finance-api"
│ goose Chat  │  ────────────────────────────────────┐
└─────────────┘                                      │
                                                     ↓
                                         ┌──────────────────┐
                                         │ goose Engine     │
                                         │ 1. Parse intent  │
                                         │ 2. Check policy  │
                                         │    (allow github)│
                                         │ 3. Call MCP tool │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ GitHub MCP       │
                                         │ Extension        │
                                         │                  │
                                         │ GET /repos/      │
                                         │  finance-api/    │
                                         │  pulls/123       │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ PR Data:         │
                                         │ • Files changed  │
                                         │ • Commits        │
                                         │ • Comments       │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
                                         ┌──────────────────┐
                                         │ goose sends to   │
                                         │ GPT-4o:          │
                                         │                  │
                                         │ "Review this PR: │
                                         │  [diff content]  │
                                         │  Context: TDD    │
                                         │  (from hints)"   │
                                         └────────┬─────────┘
                                                  │
                                                  ↓
┌─────────────┐                        ┌──────────────────┐
│ Response:   │                        │ GPT-4o:          │
│ "PR #123    │  <──────────────────── │ "LGTM. Tests pass│
│  approved.  │                         │  Code follows    │
│  Suggest    │                         │  TDD. Suggest    │
│  merge."    │                         │  merge."         │
└─────────────┘                        └──────────────────┘

Profile Features Applied:
✅ Primary: GPT-4o (complex analysis)
✅ Worker: GPT-4o-mini (simple summaries)
✅ Extensions: github (PR access), developer tools (shell, analyze)
✅ Policies: allow_tool "developer__shell" (can run git commands)
✅ Goosehints: "Use TDD" (injected into prompts)
✅ Privacy: Standard (mask PII in PR comments if detected)

Phase 3 Multi-Agent Enhancement:
⏳ "Ask Security to review this PR" (route to security profile)
⏳ "Request Legal approval for license change"
```

---

## View 3: Data Flow - Profile Loading

### Complete Profile Loading Sequence

```
┌──────────────────────────────────────────────────────────────────────┐
│                 DATA FLOW: USER LOADS PROFILE                         │
│                 Endpoint: GET /profiles/{role}                        │
└──────────────────────────────────────────────────────────────────────┘

  ┌──────────────┐
  │  goose       │  Step 1: Request with JWT
  │  Desktop     │  ──────────────────────────────────────────┐
  │              │                                            │
  │  Request:    │                                            │
  │  GET /profiles/finance                                   │
  │  Authorization: Bearer eyJhbGc...                         │
  └──────────────┘                                            ↓
                                                   ┌────────────────────┐
                                                   │  Controller        │
                                                   │  Middleware:       │
                                                   │  1. Extract JWT    │
                                                   │     from header    │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  2. Verify JWT     │
                                                   │     Signature      │
                                                   │                    │
                                                   │  • Fetch JWKS from │
                                                   │    Keycloak:       │
                                                   │    GET /realms/dev/│
                                                   │    protocol/openid-│
                                                   │    connect/certs   │
                                                   │  • Verify RS256    │
                                                   │  • Check exp       │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  3. Extract Claims │
                                                   │                    │
                                                   │  jwt_claims = {    │
                                                   │    sub: "user-id"  │
                                                   │    email: "john@..." │
                                                   │    roles: ["fin"]  │
                                                   │    exp: 1699999999 │
                                                   │  }                 │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  4. Match Role     │
                                                   │                    │
                                                   │  requested_role =  │
                                                   │   "finance"        │
                                                   │  user_roles =      │
                                                   │   ["fin", "user"]  │
                                                   │                    │
                                                   │  Mapping:          │
                                                   │  "fin" → "finance" │
                                                   │  "legal" → "legal" │
                                                   │  "dev" → "developer│
                                                   │                    │
                                                   │  ✅ Authorized     │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  5. Query Database │
                                                   │                    │
                                                   │  SELECT config     │
                                                   │  FROM profiles     │
                                                   │  WHERE role =      │
                                                   │   'finance'        │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                         ┌────────────────┴────────────────┐
                                         │  PostgreSQL: orchestrator DB    │
                                         │  Table: profiles                │
                                         │                                 │
                                         │  role='finance'                 │
                                         │  data={  ← JSONB column         │
                                         │    "providers": {               │
                                         │      "primary": "claude-3.5-    │
                                         │       sonnet",                  │
                                         │      "planner": "claude-3.5-    │
                                         │       sonnet",                  │
                                         │      "worker": "gpt-4o-mini",   │
                                         │      "allowed_providers": [     │
                                         │        "openai", "anthropic"    │
                                         │      ],                         │
                                         │      "forbidden_providers": [], │
                                         │      "temperature": 0.3         │
                                         │    },                           │
                                         │    "extensions": [              │
                                         │      {                          │
                                         │        "name": "github",        │
                                         │        "tools": ["*"],          │
                                         │        "preferences": {}        │
                                         │      }                          │
                                         │    ],                           │
                                         │    "goosehints": {              │
                                         │      "global": "You are assist...",
                                         │      "local_templates": []      │
                                         │    },                           │
                                         │    "gooseignore": {             │
                                         │      "global": [                │
                                         │        "*.env", "*.key"         │
                                         │      ],                         │
                                         │      "local_templates": []      │
                                         │    },                           │
                                         │    "recipes": [],               │
                                         │    "automated_tasks": [],       │
                                         │    "policies": [                │
                                         │      {                          │
                                         │        "allow_tool": "github__*"│
                                         │        "pattern": "github__*",  │
                                         │        "reason": "Budget track" │
                                         │      }                          │
                                         │    ],                           │
                                         │    "privacy": {                 │
                                         │      "mode": "strict",          │
                                         │      "strictness": "strict",    │
                                         │      "local_only": false,       │
                                         │      "retention_days": 30,      │
                                         │      "pii_rules": [             │
                                         │        {"entity": "SSN",        │
                                         │         "action": "MASK"},      │
                                         │        {"entity": "EMAIL",      │
                                         │         "action": "MASK"}       │
                                         │      ]                          │
                                         │    },                           │
                                         │    "env_vars": {                │
                                         │      "GOOSE_ENV": "production"  │
                                         │    },                           │
                                         │    "signature": {               │
                                         │      "algorithm": "sha2-256",   │
                                         │      "vault_key": "transit/keys/│
                                         │       profile-signing",         │
                                         │      "signed_at": "2025-11-07T04│
                                         │       :29:31.058861974+00:00",  │
                                         │      "signed_by": "admin@example│
                                         │       .com",                    │
                                         │      "signature": "vault:v1:6wmf│
                                         │       S0Vo91Ga0E9BkInhWZvLJ3qQod│
                                         │       EnXhykdywB8kc="           │
                                         │    }                            │
                                         │  }                              │
                                         └────────────────┬────────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  6. Deserialize    │
                                                   │     JSONB → Struct │
                                                   │                    │
                                                   │  profile: Profile =│
                                                   │   serde_json::     │
                                                   │   from_value(      │
                                                   │     config_jsonb   │
                                                   │   )?               │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  7. Validate       │
                                                   │     (Optional)     │
                                                   │                    │
                                                   │  Phase 6 adds:     │
                                                   │  • Verify signature│
                                                   │    with Vault      │
                                                   │  • Reject if       │
                                                   │    tampered        │
                                                   │                    │
                                                   │  Phase 5:          │
                                                   │  ✅ Skip verify    │
                                                   │     (dev mode)     │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  8. Return JSON    │
                                                   │                    │
                                                   │  HTTP 200 OK       │
                                                   │  Content-Type:     │
                                                   │   application/json │
                                                   │                    │
                                                   │  Body: {profile}   │
                                                   └──────┬─────────────┘
                                                          │
  ┌──────────────┐                                       │
  │  goose       │  <────────────────────────────────────┘
  │  Desktop     │
  │              │  Step 9: Process Profile
  │  1. Store    │  ───────────────────────────────────┐
  │     profile  │                                     │
  │  2. Request  │                                     │
  │     configs  │                                     ↓
  └──────┬───────┘                          ┌────────────────────┐
         │                                  │  Additional API    │
         │                                  │  Calls:            │
         │                                  │                    │
         │  GET /profiles/finance/config   │  Returns:          │
         │  ──────────────────────────────→│  config.yaml       │
         │                                  │  (generated from   │
         │                                  │   profile)         │
         │                                  └────────┬───────────┘
         │                                           │
         │                                           │
         │  GET /profiles/finance/goosehints        │
         │  ──────────────────────────────→         │
         │                                  │  Returns:          │
         │                                  │  .goosehints       │
         │                                  │  (from profile.    │
         │                                  │   goosehints.      │
         │                                  │   global)          │
         │                                  └────────┬───────────┘
         │                                           │
         │                                           │
         │  GET /profiles/finance/gooseignore       │
         │  ──────────────────────────────→         │
         │                                  │  Returns:          │
         │                                  │  .gooseignore      │
         │                                  │  (from profile.    │
         │                                  │   gooseignore.     │
         │                                  │   global)          │
         │                                  └────────┬───────────┘
         │                                           │
         │                                           │
         │  <────────────────────────────────────────┘
         │
         ↓
  ┌──────────────┐
  │  Step 10:    │  Save to Local Filesystem
  │  Save Files  │  ──────────────────────────────────┐
  │              │                                    │
  │  Writes:     │                                    ↓
  │  • ~/.config/goose/config.yaml              ┌────────────────────┐
  │  • ~/.config/goose/.goosehints              │  Local Files:      │
  │  • ~/.config/goose/.gooseignore             │                    │
  └──────────────┘                               │  config.yaml:     │
                                                 │  [profile]         │
                                                 │  [[provider]]      │
                                                 │  provider = "claude│
                                                 │   -3.5-sonnet"     │
                                                 │  model = "claude-3.│
                                                 │   5-sonnet"        │
                                                 │                    │
                                                 │  .goosehints:      │
                                                 │  You are assisting │
                                                 │  a finance manager │
                                                 │  ...               │
                                                 │                    │
                                                 │  .gooseignore:     │
                                                 │  *.env             │
                                                 │  *.key             │
                                                 │  .git/             │
                                                 └────────────────────┘

  ┌──────────────┐
  │  Step 11:    │  goose Ready!
  │  goose       │  
  │  Desktop     │  ✅ Configured with finance profile
  │              │  ✅ Primary provider: Claude 3.5 Sonnet
  │              │  ✅ Extensions: github
  │              │  ✅ Privacy: Strict (PII masked)
  │              │  ✅ Policies: Allow github tools
  │              │  ✅ Hints: Finance-specific context
  │              │  ✅ Ignore: Protect secrets
  └──────────────┘

Latency Breakdown (Phase 5 Performance):
┌──────────────────────────────────┬─────────────┐
│ Step                             │ Latency     │
├──────────────────────────────────┼─────────────┤
│ 1-3: JWT Verification            │ ~5ms        │
│ 4: Role Matching                 │ <1ms        │
│ 5: Database Query                │ ~3ms        │
│ 6: JSONB Deserialization         │ ~2ms        │
│ 7: Validation (Phase 6)          │ N/A         │
│ 8: JSON Serialization            │ ~2ms        │
│ ───────────────────────────────────────────── │
│ Total (P50)                      │ ~13ms       │
│ Target                           │ <20ms       │
│ Performance                      │ ✅ 35% margin│
└──────────────────────────────────┴─────────────┘
```

**Key Design Decisions:**

1. **JWT-Based Auth**: No session storage, stateless verification
2. **Role Extraction**: JWT claims contain user roles, mapped to profiles
3. **JSONB Storage**: Full profile in single JSONB column (fast retrieval, GIN index)
4. **Lazy Validation**: Signature verification deferred to Phase 6 (performance)
5. **Config Generation**: Dynamic YAML/text generation from profile JSON (no duplication)

---

## View 4: Data Flow - Profile Publishing with Vault

### Complete Vault Signing Sequence (D9 Endpoint)

```
┌──────────────────────────────────────────────────────────────────────┐
│          DATA FLOW: ADMIN PUBLISHES PROFILE (Vault HMAC)             │
│          Endpoint: POST /admin/profiles/{role}/publish               │
└──────────────────────────────────────────────────────────────────────┘

  ┌──────────────┐
  │  Admin User  │  Step 1: Publish Request
  │  (Web UI or  │  ──────────────────────────────────────────┐
  │   curl)      │                                            │
  │              │  POST /admin/profiles/finance/publish     │
  │              │  Authorization: Bearer <JWT with admin role>
  └──────────────┘                                            ↓
                                                   ┌────────────────────┐
                                                   │  Controller        │
                                                   │  Middleware:       │
                                                   │  1. Verify JWT     │
                                                   │  2. Check role =   │
                                                   │     "admin"        │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓ Authorized ✅
                                                   ┌────────────────────┐
                                                   │  Handler:          │
                                                   │  publish_profile() │
                                                   │                    │
                                                   │  Step 2:           │
                                                   │  Fetch Profile     │
                                                   │  from Database     │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                         ┌────────────────┴────────────────┐
                                         │  PostgreSQL Query:              │
                                         │                                 │
                                         │  SELECT config                  │
                                         │  FROM profiles                  │
                                         │  WHERE role = 'finance'         │
                                         │                                 │
                                         │  Returns: config JSONB          │
                                         └────────────────┬────────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  Step 3:           │
                                                   │  Prepare Data      │
                                                   │  for Signing       │
                                                   │                    │
                                                   │  1. Remove old sig │
                                                   │     (if exists)    │
                                                   │  2. Serialize to   │
                                                   │     canonical JSON │
                                                   │  3. Sort keys      │
                                                   │     (deterministic)│
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  Canonical JSON:   │
                                                   │                    │
                                                   │  {                 │
                                                   │    "automated_tasks│
                                                   │     ": [],         │
                                                   │    "description":  │
                                                   │     "Finance Mgr", │
                                                   │    "display_name": │
                                                   │     "Finance Mgr", │
                                                   │    "env_vars": {}, │
                                                   │    "extensions": [ │
                                                   │      ...           │
                                                   │    ],              │
                                                   │    "goosehints": { │
                                                   │      ...           │
                                                   │    },              │
                                                   │    ... (sorted)    │
                                                   │  }                 │
                                                   │                    │
                                                   │  No whitespace     │
                                                   │  variations!       │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  Step 4:           │
                                                   │  Encode BASE64     │
                                                   │                    │
                                                   │  input = base64::  │
                                                   │   encode(json_str) │
                                                   │                    │
                                                   │  Example:          │
                                                   │  "eyJhdXRvbWF0ZWRf│
                                                   │   dGFza3MiOltdLC...│
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
                                                   ┌────────────────────┐
                                                   │  Step 5:           │
                                                   │  Call Vault API    │
                                                   │                    │
                                                   │  POST /v1/transit/ │
                                                   │   hmac/profile-    │
                                                   │   signing/sha2-256 │
                                                   │                    │
                                                   │  Headers:          │
                                                   │  X-Vault-Token:    │
                                                   │   $VAULT_TOKEN     │
                                                   │                    │
                                                   │  Body:             │
                                                   │  {                 │
                                                   │    "input": "eyJh..│
                                                   │  }                 │
                                                   └──────┬─────────────┘
                                                          │
                                                          ↓
┌──────────────────────────────────────────────────────────────────────┐
│                          VAULT TRANSIT ENGINE                         │
│                                                                       │
│  ┌────────────────────────────────────────────────────────────────┐ │
│  │  Step 6: Vault HMAC Computation                                │ │
│  │                                                                 │ │
│  │  1. Authenticate Request:                                      │ │
│  │     • Verify X-Vault-Token (Phase 5: root token)               │ │
│  │     • Phase 6: Verify AppRole client token                     │ │
│  │                                                                 │ │
│  │  2. Decode Input:                                              │ │
│  │     • BASE64 decode → canonical JSON string                    │ │
│  │                                                                 │ │
│  │  3. Retrieve Key:                                              │ │
│  │     • Load transit/keys/profile-signing                        │ │
│  │     • Key type: aes256-gcm96 (default for Transit)             │ │
│  │     • Key version: latest (v1)                                 │ │
│  │                                                                 │ │
│  │  4. Compute HMAC:                                              │ │
│  │     • Algorithm: HMAC-SHA256                                   │ │
│  │     • Message: canonical JSON (decoded)                        │ │
│  │     • Key: transit key derivation                              │ │
│  │                                                                 │ │
│  │     hmac = HMAC-SHA256(                                        │ │
│  │       key = transit_key,                                       │ │
│  │       message = canonical_json                                 │ │
│  │     )                                                           │ │
│  │                                                                 │ │
│  │  5. Encode Signature:                                          │ │
│  │     • Format: vault:v{version}:{base64_hmac}                   │ │
│  │     • Example: vault:v1:6wmfS0Vo91Ga0E9BkInhWZvLJ3qQodEnXhykdy │
│  │                wB8kc=                                          │ │
│  │                                                                 │ │
│  │  6. Return Response:                                           │ │
│  │     {                                                           │ │
│  │       "data": {                                                │ │
│  │         "hmac": "vault:v1:6wmfS0Vo91Ga0E9BkInhWZvLJ3qQodEnXhy │
│  │                  kdywB8kc="                                    │ │
│  │       }                                                         │ │
│  │     }                                                           │ │
│  └────────────────────────────────────────────────────────────────┘ │
│                                                                       │
│  Key Management (Phase 5 Dev Mode):                                  │
│  • Storage: In-memory (ephemeral)                                    │
│  • Access: Root token (unrestricted)                                 │
│  • Audit: None                                                       │
│                                                                       │
│  Phase 6 Production:                                                 │
│  • Storage: Raft (persistent, 3 replicas)                            │
│  • Access: AppRole (controller-only policy)                          │
│  • Audit: File device (/vault/logs/audit.log)                        │
│  • TLS: HTTPS with cert verification                                 │
└───────────────────────────────────────────┬───────────────────────────┘
                                            │
                                            ↓ vault:v1:6wmfS0Vo...
                                     ┌────────────────────┐
                                     │  Step 7:           │
                                     │  Build Signature   │
                                     │  Metadata          │
                                     │                    │
                                     │  signature = {     │
                                     │    algorithm:      │
                                     │     "sha2-256",    │
                                     │    vault_key:      │
                                     │     "transit/keys/ │
                                     │      profile-      │
                                     │      signing",     │
                                     │    signed_at:      │
                                     │     now_utc(),     │
                                     │    signed_by:      │
                                     │     jwt.email,     │
                                     │    signature:      │
                                     │     vault_response │
                                     │  }                 │
                                     └──────┬─────────────┘
                                            │
                                            ↓
                                     ┌────────────────────┐
                                     │  Step 8:           │
                                     │  Update Database   │
                                     │                    │
                                     │  UPDATE profiles   │
                                     │  SET config =      │
                                     │   jsonb_set(       │
                                     │     config,        │
                                     │     '{signature}', │
                                     │     signature_json │
                                     │   ),               │
                                     │   updated_at =     │
                                     │    NOW()           │
                                     │  WHERE role =      │
                                     │   'finance'        │
                                     └──────┬─────────────┘
                                            │
                                            ↓
                          ┌─────────────────┴─────────────────┐
                          │  PostgreSQL: profiles table       │
                          │                                   │
                          │  role='finance'                   │
                          │  config={                         │
                          │    ... (all profile fields)       │
                          │    "signature": {                 │
                          │      "algorithm": "sha2-256",     │
                          │      "vault_key": "transit/keys/  │
                          │       profile-signing",           │
                          │      "signed_at": "2025-11-07T04: │
                          │       29:31.058861974+00:00",     │
                          │      "signed_by": "admin@example. │
                          │       com",                       │
                          │      "signature": "vault:v1:6wmfS0│
                          │       Vo91Ga0E9BkInhWZvLJ3qQodEnXh│
                          │       ykdywB8kc="                 │
                          │    }                              │
                          │  }                                │
                          │  updated_at='2025-11-07 04:29:31' │
                          └─────────────────┬─────────────────┘
                                            │
                                            ↓
                                     ┌────────────────────┐
                                     │  Step 9:           │
                                     │  Return Response   │
                                     │                    │
                                     │  HTTP 200 OK       │
                                     │  {                 │
                                     │    "message":      │
                                     │     "Profile pub-  │
                                     │      lished",      │
                                     │    "role": "fin",  │
                                     │    "signature": {  │
                                     │      ...           │
                                     │    }               │
                                     │  }                 │
                                     └──────┬─────────────┘
                                            │
  ┌──────────────┐                         │
  │  Admin User  │  <──────────────────────┘
  │              │
  │  Receives:   │  ✅ Profile published successfully
  │  • Signature │  ✅ Tamper protection active
  │  • Metadata  │  ✅ Signed by admin@example.com
  │              │  ✅ Timestamp: 2025-11-07 04:29:31 UTC
  └──────────────┘

Signature Verification (Phase 6):
┌──────────────────────────────────────────────────────────────────────┐
│  When User Loads Profile (GET /profiles/finance):                    │
│                                                                       │
│  1. Fetch profile from DB (includes signature)                       │
│  2. Extract signature.signature value (vault:v1:...)                 │
│  3. Remove signature field from config                               │
│  4. Serialize config to canonical JSON (same as publish)             │
│  5. Call Vault verify endpoint:                                      │
│     POST /v1/transit/verify/profile-signing/sha2-256                 │
│     {                                                                 │
│       "input": "eyJh...", (BASE64 canonical JSON)                    │
│       "hmac": "vault:v1:6wmfS0Vo..."                                 │
│     }                                                                 │
│  6. Vault returns: {"valid": true}                                   │
│  7. If valid=false: Return 403 Forbidden ("Profile tampered")        │
│  8. If valid=true: Return profile to user                            │
│                                                                       │
│  Tamper Detection:                                                   │
│  • Manual DB edit of config → HMAC mismatch → 403                    │
│  • Signature deletion → Missing signature → 403                      │
│  • Old signature reuse → Content changed → HMAC mismatch → 403       │
└───────────────────────────────────────────────────────────────────────┘

Latency Breakdown (Publish Operation):
┌──────────────────────────────────────┬─────────────┐
│ Step                                 │ Latency     │
├──────────────────────────────────────┼─────────────┤
│ 1: JWT Verification                  │ ~5ms        │
│ 2: Fetch Profile (DB query)          │ ~3ms        │
│ 3: Serialize to JSON                 │ ~2ms        │
│ 4: BASE64 Encode                     │ <1ms        │
│ 5: HTTP Call to Vault                │ ~8ms        │
│ 6: Vault HMAC Computation            │ ~3ms        │
│ 7: Build Metadata                    │ <1ms        │
│ 8: Update Database (JSONB set)       │ ~4ms        │
│ 9: Response Serialization            │ ~1ms        │
│ ────────────────────────────────────────────────── │
│ Total (P50)                          │ ~27ms       │
│ Target                               │ <50ms       │
│ Performance                          │ ✅ 46% margin│
└──────────────────────────────────────┴─────────────┘
```

**Security Guarantees:**

1. **Tamper Detection**: Any config modification → HMAC mismatch → Rejected
2. **Integrity**: Vault key never leaves Vault (HSM-equivalent in prod)
3. **Auditability**: Metadata tracks who signed, when
4. **Non-Repudiation**: Signature proves admin@example.com published
5. **Forward Secrecy**: Phase 6 key rotation invalidates old signatures

**Phase 6 Production Hardening:**

| Feature | Phase 5 (Dev Mode) | Phase 6 (Production) |
|---------|-------------------|----------------------|
| **Transport** | HTTP | HTTPS (TLS 1.3) |
| **Authentication** | Root token (static) | AppRole (rotatable secret_id) |
| **Storage** | In-memory (ephemeral) | Raft (persistent, 3 replicas) |
| **Audit** | None | File device + log rotation |
| **Verification** | Skipped (performance) | Enforced on profile load |
| **Key Protection** | Memory only | Encrypted at rest (Raft) |
| **Unseal** | Auto-unsealed | 3 of 5 keys (Shamir) |

---

## Source Code Architecture

### Service vs. Module Pattern

The `/src` directory contains 6 components organized by architectural pattern:

**Services** (Standalone Docker containers):
1. **controller/** — Main API server (port 8088, Rust/Axum)
2. **privacy-guard/** — PII detection/masking service (port 8089, Rust)

**Modules** (Rust libraries imported by controller):
3. **lifecycle/** — Session state machine (imported via `lib.rs`)
4. **profile/** — Profile schema/validation/signing (imported via `lib.rs`)
5. **vault/** — HashiCorp Vault client (imported via `lib.rs` and profile module)

**MCP Extensions** (Launched by goose Desktop):
6. **agent-mesh/** — Multi-agent coordination (Python MCP stdio)

### Import Pattern

Controller imports modules via `src/controller/src/lib.rs`:
```rust
// Phase 4: Lifecycle management (lives outside controller for reusability)
#[path = "../../lifecycle/mod.rs"]
pub mod lifecycle;

// Phase 5: Vault client (production-grade HashiCorp Vault integration)
#[path = "../../vault/mod.rs"]
pub mod vault;

// Phase 5: Profile system
#[path = "../../profile/mod.rs"]
pub mod profile;
```

**Usage Examples:**

**lifecycle** — Session state machine (Phase 4):
- **Purpose**: Enforce valid state transitions (pending → active → completed/failed/expired)
- **Status**: Infrastructure ready (Phase 6 wiring)
- **Current**: Imported but not yet called in routes (direct DB access in Phase 4)
- **Future**: Phase 6 will wire lifecycle into routes for validation

**profile** — Used in route handlers:
```rust
// src/controller/src/routes/profiles.rs
use crate::profile::schema::Profile;

let profile: Profile = serde_json::from_value(data)?;
Ok(Json(profile))
```

**vault** — Used by admin routes and profile module:
```rust
// src/controller/src/routes/admin/profiles.rs
use crate::vault::transit::TransitOps;

let transit = TransitOps::new(vault_client);
let signature = transit.sign_data(&profile_data).await?;
```

See **[/src Architecture Audit](SRC-ARCHITECTURE-AUDIT.md)** for complete analysis.

---

## Component Details

### Keycloak (Authentication)

**Role**: OIDC Provider (OAuth 2.0 + OpenID Connect)

**Configuration**:
- Realm: `dev`
- Client: `goose-controller` (PUBLIC client - no client_secret required)
- Flow: Password Grant only (user login with username/password)
  - **Note**: Client Credentials flow NOT supported (public client type)
  - Use `grant_type=password` with user credentials
- Token: JWT (RS256 signature)

**Token Acquisition Example**:
```bash
# Correct (Password Grant)
curl -X POST http://localhost:8080/realms/dev/protocol/openid-connect/token \
  -d "grant_type=password" \
  -d "client_id=goose-controller" \
  -d "username=testuser" \
  -d "password=testpassword"

# Will fail (Client Credentials not supported for public clients)
curl -X POST http://localhost:8080/realms/dev/protocol/openid-connect/token \
  -d "grant_type=client_credentials" \
  -d "client_id=goose-controller" \
  -d "client_secret=..." # No secret for public clients
```

**Users**:
```
testuser / testpassword
  Roles: ["finance", "user"]
  Email: testuser@example.com

admin@example.com
  Roles: ["admin", "user"]
  Email: admin@example.com
```

**Token Claims**:
```json
{
  "sub": "user-id-uuid",
  "email": "testuser@example.com",
  "roles": ["finance", "user"],
  "exp": 1699999999,
  "iat": 1699996399,
  "iss": "http://localhost:8080/realms/dev"
}
```

**JWKS Endpoint**: `http://localhost:8080/realms/dev/protocol/openid-connect/certs`

---

### Privacy Guard (PII Masking)

**Role**: Local PII detection and masking middleware

**Capabilities**:
- 8 entity types: SSN, Email, Phone, Credit Card, Person, IP, DOB, Account
- 25+ regex detection patterns
- Deterministic HMAC-SHA256 pseudonymization
- Format-preserving encryption (FPE) for structured data

**Performance**:
- P50: 16ms
- P95: 22ms
- P99: 23ms
- Target: <700ms (31x better than target)

**Modes**:
- OFF: No masking
- DETECT: Scan only (log entities)
- MASK: Mask PII (default)
- STRICT: Mask + block cloud if local_only=true

**Storage**:
- Local tokens: `~/.goose/pii-tokens/` (ephemeral, per-session)
- No cloud storage (privacy-first)

**Integration**: MCP extension (optional, enabled per profile)

---

### Controller (Orchestration)

**Role**: Central API server (profile management, org chart, audit)

**Language**: Rust (Axum framework)

**Endpoints**: 13 total (D1-D12, E5)

**Middleware**:
- JWT verification (RS256 + JWKS)
- Role extraction (from JWT claims)
- Request logging (structured, PII-redacted)
- CORS (Phase 6: configure for admin UI)

**Performance**:
- Profile loading: P50=13ms (target <20ms)
- Profile publishing: P50=27ms (target <50ms)
- Org chart tree: P50=18ms (target <30ms)

**Database**: PostgreSQL (JSONB for profiles, relational for org chart)

---

### Vault (HMAC Signing)

**Role**: Profile integrity via HMAC signatures

**Configuration** (Phase 5 Dev Mode):
- Image: hashicorp/vault:1.18.3
- Mode: Dev (HTTP, root token, in-memory)
- Transit Engine: Enabled via vault-init.sh
- Key: `profile-signing` (HMAC-SHA256)
- Environment: VAULT_ADDR=http://vault:8200, VAULT_TOKEN=root

**Operations**:
- Sign: POST /v1/transit/hmac/profile-signing/sha2-256
- Verify: POST /v1/transit/verify/profile-signing/sha2-256

**Signature Format**: `vault:v1:{BASE64_HMAC}`

**Phase 6 Production**:
- TLS/HTTPS (2 hours)
- AppRole auth (3 hours)
- Raft storage (2 hours)
- Audit device (1 hour)
- Signature verification (2 hours)

---

### PostgreSQL (Metadata Storage)

**Role**: Profile, org chart, audit trail storage

**Database**: `orchestrator` (not default `postgres`)

**Connection Example**:
```bash
# Correct (specify orchestrator database)
docker exec ce_postgres psql -U postgres -d orchestrator -c "SELECT role FROM profiles;"

# Wrong (will fail - profiles table doesn't exist in postgres database)
docker exec ce_postgres psql -U postgres -c "SELECT role FROM profiles;"
```

**Tables** (Phase 5):

1. **profiles** - Role-based configurations
```sql
CREATE TABLE profiles (
  role VARCHAR(255) PRIMARY KEY,
  display_name VARCHAR(255) NOT NULL,
  data JSONB NOT NULL,  -- All profile config nested here
  signature TEXT,       -- Vault HMAC signature (nullable)
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_profiles_data ON profiles USING GIN (data);

-- Example query (JSONB nested access)
SELECT 
  role,
  display_name,
  (data->>'description') as description,
  (data->'privacy'->>'mode') as privacy_mode,
  (data->'privacy'->>'retention_days')::int as retention_days
FROM profiles
WHERE role = 'finance';
```

2. **org_users** - Organizational hierarchy
```sql
CREATE TABLE org_users (
  employee_id VARCHAR(50) PRIMARY KEY,
  email VARCHAR(255) UNIQUE NOT NULL,
  name VARCHAR(255) NOT NULL,
  manager_email VARCHAR(255),  -- Self-referencing FK
  department VARCHAR(255),
  title VARCHAR(255),
  created_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_org_users_email ON org_users(email);
CREATE INDEX idx_org_users_manager ON org_users(manager_email);
CREATE INDEX idx_org_users_dept ON org_users(department);
```

3. **org_imports** - Import audit trail
```sql
CREATE TABLE org_imports (
  id SERIAL PRIMARY KEY,              -- NOT import_id
  filename VARCHAR(255) NOT NULL,
  users_created INTEGER DEFAULT 0,    -- Split metrics
  users_updated INTEGER DEFAULT 0,
  status VARCHAR(50) NOT NULL,
  imported_by VARCHAR(255),
  imported_at TIMESTAMPTZ DEFAULT NOW()
);
```

4. **privacy_audit_logs** - PII detection audit
```sql
CREATE TABLE privacy_audit_logs (
  id BIGSERIAL PRIMARY KEY,
  session_id VARCHAR(255),
  redaction_count INTEGER,
  categories TEXT[],
  mode VARCHAR(50),
  timestamp TIMESTAMPTZ,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
-- NOTE: tenant_id used in API but NOT stored in logs (privacy requirement)
```

**Legacy Tables** (Phase 1-2):
- audit_events (Phase 1)
- sessions (Phase 1)
- privacy_events (Phase 2)

**Performance**:
- Profile query: ~3ms (JSONB GIN index)
- Org tree query: ~5ms (recursive CTE with manager_email index)

---

## API Endpoint Summary

### Profile Endpoints (Workstream D)

| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/profiles/{role}` | GET | User JWT | Load full profile JSON |
| `/profiles/{role}/config` | GET | User JWT | Generate config.yaml |
| `/profiles/{role}/goosehints` | GET | User JWT | Get global hints |
| `/profiles/{role}/goosehints/local` | GET | User JWT | Get local hints template |
| `/profiles/{role}/gooseignore` | GET | User JWT | Get global ignore patterns |
| `/profiles/{role}/recipes` | GET | User JWT | List recipes |
| `/admin/profiles` | POST | Admin JWT | Create new profile |
| `/admin/profiles/{role}` | PUT | Admin JWT | Update profile (partial merge) |
| `/admin/profiles/{role}/publish` | POST | Admin JWT | Sign profile with Vault |

### Org Chart Endpoints (Workstream E)

| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/admin/org/import` | POST | Admin JWT | Upload CSV (bulk insert) |
| `/admin/org/imports` | GET | Admin JWT | Import history + audit |
| `/admin/org/tree` | GET | Admin JWT | Hierarchical org structure |

### Infrastructure Endpoints

| Endpoint | Method | Auth | Description |
|----------|--------|------|-------------|
| `/status` | GET | None | Health check |
| `/audit/ingest` | POST | JWT | Audit log ingestion (Phase 1) |

---

## Security Architecture

### Authentication Flow

```
User Login
  → Keycloak OIDC (Password Grant)
    → JWT Token (RS256 signed)
      → Controller verifies with JWKS
        → Extract role from claims
          → Match role to profile
            → Return profile
```

### Authorization Layers

1. **JWT Verification**: RS256 signature + expiration check
2. **Role Extraction**: Map JWT roles to profile roles
3. **Role-Based Access Control (RBAC)**: Admin vs User endpoints
4. **Policy Enforcement** (Phase 3): Tool allowlists/denylists per profile
5. **Vault Integration**: Profile signature verification (Phase 6)

### Privacy Protection

1. **Local-First**: PII masked before cloud LLM calls
2. **Deterministic Masking**: Same PII → Same token (reversible)
3. **Ephemeral Tokens**: Per-session storage, deleted after use
4. **No Cloud Storage**: Tokens never leave local filesystem
5. **Audit Trail**: Privacy events logged (Phase 2)

### Vault Security (Phase 6)

1. **TLS/HTTPS**: Encrypted transport
2. **AppRole Auth**: Scoped permissions (controller-only)
3. **Persistent Storage**: Raft backend (encrypted at rest)
4. **Audit Device**: All operations logged
5. **Key Rotation**: Periodic rotation (90 days)

---

## Phase 6 Preview

### Admin UI (SvelteKit)

```
┌────────────────────────────────────────────────────────────┐
│                     ADMIN WEB UI                           │
│                                                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │  Dashboard   │  │   Profiles   │  │  Org Chart   │    │
│  │              │  │              │  │              │    │
│  │  • D3.js     │  │  • Monaco    │  │  • CSV       │    │
│  │    Org Chart │  │    Editor    │  │    Import    │    │
│  │  • Live      │  │  • Schema    │  │  • Tree      │    │
│  │    Sessions  │  │    Validation│  │    Viewer    │    │
│  │  • Metrics   │  │  • Publish   │  │  • Hierarchy │    │
│  │              │  │    Button    │  │    Viz       │    │
│  └──────────────┘  └──────────────┘  └──────────────┘    │
│                                                            │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │  Audit Logs  │  │   Settings   │  │  Vault       │    │
│  │              │  │              │  │  Status      │    │
│  │  • Table     │  │  • Keycloak  │  │              │    │
│  │  • Filters   │  │  • Vault     │  │  • Sealed/   │    │
│  │  • Export    │  │  • Database  │  │    Unsealed  │    │
│  │    CSV       │  │  • Privacy   │  │  • Key       │    │
│  │              │  │    Guard     │  │    Version   │    │
│  └──────────────┘  └──────────────┘  └──────────────┘    │
│                                                            │
│  Authentication: Keycloak OIDC (admin role required)      │
│  API: Controller endpoints (D7-D9, E10-E12)               │
└────────────────────────────────────────────────────────────┘
```

### User UI (Lightweight)

```
┌────────────────────────────────────────────────────────────┐
│                     USER WEB UI (Phase 6)                  │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  My Profile                                          │ │
│  │  ┌────────────────────────────────────────────────┐ │ │
│  │  │  Role: Finance Manager                         │ │ │
│  │  │  Primary Provider: Claude 3.5 Sonnet           │ │ │
│  │  │  Extensions: github                            │ │ │
│  │  │  Privacy: Strict (PII masked)                  │ │ │
│  │  │  ┌────────────────────────────────────────┐   │ │ │
│  │  │  │  [Download config.yaml]                │   │ │ │
│  │  │  │  [Download .goosehints]                │   │ │ │
│  │  │  │  [Download .gooseignore]               │   │ │ │
│  │  │  └────────────────────────────────────────┘   │ │ │
│  │  └────────────────────────────────────────────────┘ │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  My Sessions                                         │ │
│  │  ┌────────────────────────────────────────────────┐ │ │
│  │  │  Session ID  │  Started At  │  Status  │       │ │ │
│  │  │  ────────────┼──────────────┼──────────┤       │ │ │
│  │  │  ses-123     │  2h ago      │  Active  │       │ │ │
│  │  │  ses-122     │  5h ago      │  Ended   │       │ │ │
│  │  └────────────────────────────────────────────────┘ │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  Integration: goose Desktop (download configs)            │
│  Privacy Guard: MCP middleware (optional)                 │
└────────────────────────────────────────────────────────────┘
```

### Vault Production (Phase 6)

```
┌────────────────────────────────────────────────────────────┐
│                  VAULT PRODUCTION SETUP                     │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  TLS/HTTPS Setup (2 hours)                           │ │
│  │  • Generate TLS certs (OpenSSL or Let's Encrypt)     │ │
│  │  • Update listener config (tls_cert_file)            │ │
│  │  • Update VAULT_ADDR (https://vault:8200)            │ │
│  │  • Add VAULT_CACERT (cert verification)              │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  AppRole Authentication (3 hours)                     │ │
│  │  • Enable AppRole auth method                        │ │
│  │  • Create controller-role (Transit permissions)      │ │
│  │  • Generate role_id (static), secret_id (rotatable)  │ │
│  │  • Update controller code (AppRole login)            │ │
│  │  • Implement token renewal (45-min intervals)        │ │
│  │  • Remove VAULT_TOKEN from env vars                  │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  Persistent Storage (2 hours)                         │ │
│  │  • Configure Raft storage backend (3 replicas)       │ │
│  │  • Initialize Vault (generate unseal keys)           │ │
│  │  • Unseal procedure (3 of 5 keys)                    │ │
│  │  • Automate unseal (AWS KMS optional)                │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  Audit Device (1 hour)                                │ │
│  │  • Enable file audit device                          │ │
│  │  • Mount /vault/logs volume                          │ │
│  │  • Configure log rotation (logrotate)                │ │
│  │  • Verify all operations logged                      │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  ┌──────────────────────────────────────────────────────┐ │
│  │  Signature Verification (2 hours)                     │ │
│  │  • Add verify to profile loading (GET /profiles)     │ │
│  │  • Implement verify_hmac function (Rust)             │ │
│  │  • Reject tampered profiles (403 Forbidden)          │ │
│  │  • Add verify audit logs                             │ │
│  └──────────────────────────────────────────────────────┘ │
│                                                            │
│  Total Effort: 10 hours (4-6 hours Vault + 2h UI + 2h)    │
└────────────────────────────────────────────────────────────┘
```

---

## Related Documentation

- **Profile Specification**: [`docs/profiles/SPEC.md`](../profiles/SPEC.md) - Complete schema reference
- **Vault Operations Guide**: [`docs/guides/VAULT.md`](../guides/VAULT.md) - Dev mode + Phase 6 production
- **Migration Guide**: [`docs/MIGRATION-PHASE5.md`](MIGRATION-PHASE5.md) - Upgrade from v0.4.0
- **Test Results**: [`docs/tests/phase5-test-results.md`](../tests/phase5-test-results.md) - 60/60 tests passing
- **Master Technical Plan**: [`Technical Project Plan/master-technical-project-plan.md`](../../Technical%20Project%20Plan/master-technical-project-plan.md)

---

**Last Updated**: 2025-11-06  
**Version**: v0.5.0  
**Status**: Phase 5 Complete, Phase 6 Planned
